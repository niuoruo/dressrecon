...
class VidDataset(Dataset):
    ...
    Args:
        ...
        data_prefix (str): If provided, override the given data_prefix in opts
        uncertainty (np.array): If provided, uncertainty array to be updated with loss
    """

    def __init__(self, opts, rgblist, dataid, ks, raw_size, data_prefix=None, uncertainty=None):
        if data_prefix is None:
            data_prefix = opts["data_prefix"]

        self.delta_list = opts["delta_list"]
        self.field_type = opts["field_type"]
        self.uncertainty_factor = opts["uncertainty_factor"]
        self.dict_list = self.construct_data_list(rgblist, data_prefix, opts["feature_type"])
    ...
    def construct_data_list(self, reflist, prefix, feature_type):
        ...
        Args:
            ...
            feature_type (str): Type of image features to use ("cse" or "dinov2")
        Returns:
            dict_list (Dict(str, List(str))): Maps each frame/annotation type
                to a list of .npy/.txt paths for that type
        """
        rgb_path = reflist[0].replace("00000.jpg", f"{prefix}.npy")
        ...
        normal_path = rgb_path.replace("JPEGImages", "NormalClothF")
        group_id = 0 if self.field_type == "bg" else 1
        feature_parent = str(Path(rgb_path.replace("JPEGImages", "Features")).parent)
        feature_path = f"{feature_parent}/{prefix}-{feature_type}-{group_id:02d}.npy"
        ...
        joint_angles = (
            reflist[0]
            .replace("JPEGImages", "Cameras")
            .replace("00000.jpg", "smpl_pose.npy")
        )
    ...
    def sample_xy(self, img_idx=None, with_uncertainty=True):
        """Sample random pixels from an image

        Args:
            img_idx (int): Image index to sample. Only used to sample uncertainty
            with_uncertainty (bool): If True, 50% of pixels are sampled by taking the
                maximum-uncertainty pixels from a larger pool of samples
        Returns:
        ...
        if with_uncertainty:
            # Sample some pixels randomly, and other pixels to have large uncertainty
            n_unc_px = int(self.pixels_per_image * self.uncertainty_factor)
            n_rand_px = self.pixels_per_image - n_unc_px
            rand_idx = self.idx_sampler.sample(n_rand_px + 5 * n_unc_px)

            rand_unc_idx_y = rand_idx[n_rand_px:] % self.img_size[0]
            rand_unc_idx_x = rand_idx[n_rand_px:] // self.img_size[0]
            rand_unc_idx_xy = np.stack([rand_unc_idx_x, rand_unc_idx_y], axis=-1)
            uncertainty = self.read_uncertainty(img_idx, rand_unc_idx_xy)
            
            high_unc_idxs = np.argsort(uncertainty)[::-1] + n_rand_px
            rand_idx[n_rand_px:] = rand_idx[high_unc_idxs]
            rand_idx = rand_idx[:self.pixels_per_image]
        else:
            rand_idx = self.idx_sampler.sample(self.pixels_per_image)
    ...
    def load_data(self, im0idx):
        ...
        with_uncertainty = self.uncertainty is not None
        rand_xy0 = self.sample_xy(im0idx, with_uncertainty=with_uncertainty)
        rand_xy1 = self.sample_xy(im1idx, with_uncertainty=with_uncertainty)
    ...
    def read_feature(self, im0idx, rand_xy=None):
        ...
        Args:
            ...
            rand_xy (np.array or None): (N,2) Pixels to load, if given
        Returns:
            feat (np.array): (H,W,16) or (N,16) Feature map, float32
        """
        feat = self.mmap_list["feature"][im0idx]
        if rand_xy is None:
    ...
    def read_uncertainty(self, im0idx, rand_xy=None):
        """Read uncertainty map for a single frame

        Args:
            im0idx (int): Frame id to load
            rand_xy (np.array or None): (N,2) Pixels to load, if given
        Returns:
            uncertainty (np.array): (H,W) or (N,) Uncertainty map, float16
        """
        if self.uncertainty is None:
            raise NotImplementedError
        uncertainty = self.uncertainty[im0idx]
        if rand_xy is None:
            uncertainty = cv2.resize(uncertainty.astype(np.float32), self.res)
        else:
            uncertainty = uncertainty[rand_xy[:, 1], rand_xy[:, 0]]
