...
class CameraConst(nn.Module):
    ...
    def get_vals(self, frame_id=None):
        ...
        if frame_id is None:
            ...
            quat = self.quat[frame_id].to(frame_id.device)
            trans = self.trans[frame_id].to(frame_id.device)
        return quat, trans

    def compute_distance_to_prior(self):
        return torch.tensor([0.], device=self.trans.device)
...
class CameraMLP_so3(TimeMLP):
    ...
    def __init__(
        ...
        self.frame_mapping = self.time_embedding.frame_mapping
...
class ArticulationFlatMLP(ArticulationBaseMLP):
    ...
    def skel_prior_loss(self):
        ...
        """
        device = self.parameters().__next__().device
        empty_feat = torch.zeros(1, 0, device=device)
        # encourage bones to be near average log bone length
...
class ArticulationSkelMLP(ArticulationBaseMLP):
    ...
    def joint_prior_loss(self):
        ...
        """
        inst_id = self.time_embedding.frame_to_vid
        so3_gt = self.init_vals  # N,J,3

        t_embed = self.time_embedding(frame_id=None)
        so3 = self.forward(t_embed, inst_id, return_so3=True)
        loss = F.mse_loss(so3, so3_gt)
        return loss
...
class ArticulationURDFMLP(ArticulationSkelMLP):
    ...
    def __init__(
        ...
        if joint_angles is not None:
            if skel_type == "smpl":
                # "urdf-smpl" is written assuming OpenGL coordinates.
                # However, our data processing script outputs OpenCV coordinates for all
                # joint angles, so need to convert back to OpenGL here.
                # Note this does not apply to "skel-smpl" which assumes OpenCV coordinates.
                joint_angles[:, :, (1, 2)] *= -1
    ...
    def parse_urdf(self, urdf_name):
        ...
        urdf_path = f"database/urdf_templates/{urdf_name}.urdf"
        urdf = URDF.load(urdf_path)

        local_rest_coord_labels = [i.name for i in urdf.joints][::3]
        local_rest_coord = np.stack([i.origin for i in urdf.joints], 0)[::3]

        if urdf_name == "smpl":
            offset = torch.tensor([0.0, -0.04, 0.0])
            orient = torch.tensor([0.0, -1.0, 0.0, 0.0])  # wxyz
            scale_factor = torch.tensor([0.2])
        elif urdf_name == "human":
        ...
        bone_centers = torch.stack(bone_centers[1:], dim=0)  # skip root
        bone_sizes = torch.stack(bone_sizes[1:], dim=0)  # skip root
        link_labels = link_labels[1:]
    ...
    def shift_joints_to_bones(self, bone_to_obj):
        ...
        idn_quat[..., 0] = 1.0
        bone_centers = self.bone_centers * self.logscale.exp().clone()
        bone_centers = bone_centers.expand_as(idn_quat[..., :3])
