...
class NeRF(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("near_far", torch.zeros(frame_offset_raw[-1], 2), persistent=False)
    ...
    def forward(self, xyz, dir=None, frame_id=None, inst_id=None):
        ...
        if dir is not None:
            ...
            if self.appr_channels > 0:
                ...
                appr_embed = torch.cat([dir_embed, appr_embed], dim=-1)
            else:
            ...
            rgb = self.rgb(torch.cat([xyz_feat, appr_embed], dim=-1))
            if self.color_act:
                rgb = rgb.sigmoid()
            out = (rgb,) + out
        return out
    ...
    def geometry_init(self, sdf_fn, nsample=4096):
        ...
        # optimize
        for i in tqdm.trange(1000, desc="Initializing SDF geometry"):
            optimizer.zero_grad()
    ...
    def extract_canonical_color(self, mesh, frame_id=None):
        ...
        color = self.forward(verts[:, None, None], dir=dir[:, None, None], frame_id=frame_id)[0][:, 0, 0]
    ...
    def update_near_far(self, beta=0.9):
        ...
        if verts is not None:
            ...
            frame_mapping = self.camera_mlp.frame_mapping
    ...
    def compute_eikonal(self, xyz, inst_id=None, sample_ratio=16):
        ...
        Returns:
            ...
            normal: (M,N,D,3) Normal vector field in canonical space
        """
        M, N, D, _ = xyz.shape
        sample_size = (M * N + sample_ratio - 1) // sample_ratio

        # subsample random pixels to reduce computation + memory usage
        if M * N > sample_size:
            eikonal_loss = torch.zeros_like(xyz[..., 0]).reshape(M * N, 1, D, 1)  # M*N, 1, D, 1
            normal = torch.zeros_like(xyz).reshape(M * N, 1, D, 3)  # M*N, 1, D, 3

            # duplicate dataset along pixel dimension
            xyz = xyz.reshape(M * N, 1, D, 3)  # M*N, 1, D, 3
            if inst_id is not None:
                inst_id = inst_id[:, None].expand(M, N).reshape(M * N)  # M*N,

            # sample random pixels
            probs = torch.ones(M * N, device=xyz.device)  # M*N,
            rand_inds = torch.multinomial(probs, sample_size, replacement=False)  # M*N,
            xyz = xyz[rand_inds]  # M*N/ratio, 1, D, 3
            if inst_id is not None:
                inst_id = inst_id[rand_inds]  # M*N/ratio,
        ...
        g = compute_gradients_sdf(fn_sdf, xyz, training=self.training)  # M*N/ratio, 1, D, 3
        g_norm = g.norm(2, dim=-1, keepdim=True)  # M*N/ratio, 1, D, 1

        eikonal_out = (g_norm - 1) ** 2
        # Zero out normals that correspond to a high-magnitude eikonal loss
        # Added for more stable training when normals are through a deformation field
        normal_out = torch.where(g_norm > 10, 0, self.grad_to_normal(g))

        if M * N > sample_size:
            eikonal_loss[rand_inds] = eikonal_out
            normal[rand_inds] = normal_out
        else:
            eikonal_loss = eikonal_out
            normal = normal_out
    ...
    def compute_eikonal_view(
        self, xyz_cam, dir_cam, field2cam, frame_id=None, inst_id=None, samples_dict={}, sample_ratio=16
    ):
        ...
        # subsample random pixels to reduce computation + memory usage
        if M * N > sample_size:
            eikonal_loss = torch.zeros_like(xyz_cam[..., 0]).reshape(M * N, 1, D, 1)  # M*N, 1, D, 1
            normal = torch.zeros_like(xyz_cam).reshape(M * N, 1, D, 3)  # M*N, 1, D, 3

            # duplicate dataset along pixel dimension
            xyz_cam = xyz_cam.reshape(M * N, 1, D, 3)  # M*N, 1, D, 3
            dir_cam = dir_cam.reshape(M * N, 1, D, 3)  # M*N, 1, D, 3
            field2cam = (
                field2cam[0][:, None].expand(M, N, 4).reshape(M * N, 4),
                field2cam[1][:, None].expand(M, N, 3).reshape(M * N, 3),
            )  # M*N, SE(3)
            if frame_id is not None:
                frame_id = frame_id[:, None].expand(M, N).reshape(M * N)  # M*N,
            if inst_id is not None:
                inst_id = inst_id[:, None].expand(M, N).reshape(M * N)  # M*N,
            samples_dict = {
                k: (
                    tuple(
                        vv[:, None].expand(M, N, *vv.shape[1:]).reshape(M * N, *vv.shape[1:])
                        for vv in v
                    )
                    if isinstance(v, tuple)
                    else v[:, None].expand(M, N, *v.shape[1:]).reshape(M * N, *v.shape[1:])
                )
                for k, v in samples_dict.items()
            }

            # sample random pixels
            probs = torch.ones(M * N, device=xyz_cam.device)  # M*N,
            rand_inds = torch.multinomial(probs, sample_size, replacement=False)  # M*N,
            xyz_cam = xyz_cam[rand_inds]  # M*N/ratio, 1, D, 3
            dir_cam = dir_cam[rand_inds]  # M*N/ratio, 1, D, 3
            field2cam = (field2cam[0][rand_inds], field2cam[1][rand_inds])  # M*N/ratio, SE(3)
            if frame_id is not None:
                frame_id = frame_id[rand_inds]  # M*N/ratio,
            if inst_id is not None:
                inst_id = inst_id[rand_inds]  # M*N/ratio,
            samples_dict = {
                k: (tuple(vv[rand_inds] for vv in v) if isinstance(v, tuple) else v[rand_inds])
                for k, v in samples_dict.items()
            }

        xyz_cam = xyz_cam.detach()
        # Note: Should NOT detach the camera and other inputs.
        # Doing so makes camera optimization unstable, empirically cameras tend to fall into the center
        ...
        def fn_sdf(xyz_cam):
            ...
            # Note: Should NOT detach xyz after backward warp, even though it's faster.
            # Doing so makes camera optimization unstable, empirically cameras tend to fall into the center
            # xyz = xyz.detach()
        ...
        g = compute_gradients_sdf(fn_sdf, xyz_cam, training=self.training)  # M*N/ratio, 1, D, 3
        g_norm = g.norm(2, dim=-1, keepdim=True)  # M*N/ratio, 1, D, 1

        eikonal_out = (g_norm - 1) ** 2
        # Zero out normals that correspond to a high-magnitude eikonal loss
        # Added for more stable training when normals are through a deformation field
        normal_out = self.grad_to_normal(g)
    ...
    def grad_to_normal(g):
        ...
        # Multiply by [1, -1, -1] for CV <=> GL convention
    ...
    def get_valid_idx(self, xyz, xyz_t=None, vis_score=None, samples_dict={}):
        ...
        if xyz_t is not None and "t_articulation" in samples_dict.keys():
            ...
            t_bones = dual_quaternion_to_quaternion_translation(samples_dict["t_articulation"])[1][0]
            t_aabb = torch.stack([t_bones.min(0)[0], t_bones.max(0)[0]], dim=0)
    ...
    def importance_sampling(
        ...
        with torch.no_grad():
            ...
            depth, _ = torch.sort(torch.cat([depth, depth_], dim=-2), dim=-2)  # (M, N, D, 1)
    ...
    def compute_jacobian(
        ...
        Args:
            ...
            samples_dict (Dict): Time-dependent bone articulations. Only for rendering. Keys:
                ...
                "t_articulation": ((M,B,4), (M,B,4))
        Returns:
            jacob_dict (Dict): Jacobian fields. Keys: "eikonal" (M,N,D,1) and "normal" (M,N,D,3)
        """
        jacob_dict = {}
        sample_ratio_canon = 16 if self.training else 1
        sample_ratio_view = 2 if self.training else 1

        eikonal_canon, normal_canon = self.compute_eikonal(xyz, inst_id, sample_ratio_canon)
        eikonal_view, normal_view = self.compute_eikonal_view(
            xyz_cam, dir_cam, field2cam, frame_id, inst_id, samples_dict, sample_ratio_view
        )
        jacob_dict["eikonal"] = eikonal_view + eikonal_canon
        jacob_dict["normal"] = normal_view
        return jacob_dict
    ...
    def query_nerf(self, xyz, dir, frame_id, inst_id, valid_idx=None):
        ...
        if valid_idx is not None:
            if valid_idx.sum() == 0:
                field_dict = {
                    ...
                    f"density_{self.category}": torch.zeros(valid_idx.shape + (1,), device=xyz.device),
        ...
        field_dict = {
            ...
            "sdf": sdf,
            "density": density,
            f"density_{self.category}": (density / self.logibeta.exp()).detach(),  # (0,1)
        }
    ...
    def wipe_loss(self, nsample=512):
        ...
        aabb_sub_min = aabb[..., 0, :] + torch.rand_like(aabb_size) * (aabb_size - aabb_size_sub)
        aabb_sub_max = aabb_sub_min + aabb_size_sub
        aabb_sub = torch.stack([aabb_sub_min, aabb_sub_max], dim=-2)
    ...
    def cam_to_field(xyz_cam, dir_cam, field2cam):
        ...
        if len(cam2field[0].shape) != len(xyz_cam.shape):
            cam2field = (
                cam2field[0][:, None, None].expand(shape[:-1] + (4,)).contiguous(),
                cam2field[1][:, None, None].expand(shape[:-1] + (3,)).contiguous(),
            )
    ...
    def field_to_cam(self, xyz, field2cam):
        ...
        shape = xyz.shape
        field2cam = (
            field2cam[0][:, None, None].expand(shape[:-1] + (4,)).contiguous(),
            field2cam[1][:, None, None].expand(shape[:-1] + (3,)).contiguous(),
        )
    ...
    def cycle_loss(self, xyz, xyz_t, frame_id, inst_id, samples_dict={}):
        ...
        This is zero for static NeRFs, and is overwritten in Deformable.
