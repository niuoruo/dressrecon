...
class IdentityWarp(nn.Module):
    ...
    def forward(
        self, xyz, frame_id, inst_id, type="forward", samples_dict={},
        return_aux=False, disable_post_warp=False,
    ):
        """
        Args:
...
class SkinningWarp(IdentityWarp):
    ...
    def forward(
        ...
        articulation = (
            articulation[0][:, None, None, :, :],  # M, N=1, D=1, B, 4
            articulation[1][:, None, None, :, :],  # M, N=1, D=1, B, 4
        )
    ...
    def get_xyz_bone_distance(self, xyz, bone2obj=None):
        ...
        Args:
            xyz: (N, 3) Points in object canonical space
            bone2obj: ((1/N,B,4), (1/N,B,4)) Bone-to-object SE(3) transforms,
        Returns:
        ...
        if isinstance(self.articulation, ArticulationURDFMLP):
            ...
            bone2obj = (
                bone2obj[0][:, None, None],  # 1, 1, 1, B, 4
                bone2obj[1][:, None, None],  # 1, 1, 1, B, 4
            )  # (N,1,1,K,4)
    ...
    def get_template_vis(self, aabb):
        ...
        mesh_sdf = marching_cubes(sdf_func, aabb, grid_size=128, level=0.0)
...
class ComposedWarp(SkinningWarp):
    ...
    def __init__(
        ...
        assert type_list[0] in ["skel-smpl", "skel-human", "skel-quad", "urdf-smpl", "urdf-human", "urdf-quad"]
        assert type_list[1] in ["bob", "dense", "nvp"]
        ...
        if type_list[1] == "bob":
            self.post_warp = SkinningWarp(frame_info, skel_type="flat", init_gauss_scale=0.01)
        elif type_list[1] == "dense":
            self.post_warp = DenseWarp(frame_info, D=2, W=256)
        elif type_list[1] == "nvp":
    ...
    def forward(
        ...
        Args:
            ...
            samples_dict: Time-dependent bone articulations. Keys:
                "rest_articulation": ((M,B,4), (M,B,4)) and
                "t_articulation": ((M,B,4), (M,B,4))
            disable_post_warp (bool): If passed, disable post-warp for visualization
        Returns:
            out: (M,N,D,3) Warped xyz coordinates
        """
        assert frame_id is not None, f"Frame ids should be specified during {type} warping"

        if type == "flow":
            raise NotImplementedError

        if (type == "forward" or type == "flow") and (not disable_post_warp):
            # Note: We first forward deform by clothing layer gaussians so that they can
            # act on the body at its mean pose. Then we deform by body layer gaussians.
            # We do not pass samples_dict as samples_dict is intended to cache the
            # body-layer skeleton articulations.
            xyz = self.post_warp.forward(xyz, frame_id, inst_id, type="forward")
        ...
        if (type == "backward" or type == "flow") and (not disable_post_warp):
            # Note: We first backward deform by body layer gaussians so that applying the
            # clothing layer backward deformation returns the body to its mean pose.
            # We do not pass samples_dict as samples_dict is intended to cache the
            # body-layer skeleton articulations.
            out = self.post_warp.forward(out, frame_id, inst_id, type="backward")
    ...
    def compute_post_warp_dist2(self, xyz, frame_id, inst_id):
        ...
        # Not needed to enforce this for NVPWarp, as it is already invertible
        if not isinstance(self.post_warp, NVPWarp):
