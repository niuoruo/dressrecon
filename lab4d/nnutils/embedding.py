...
class PosEmbedding(nn.Module):
    ...
    Args:
        in_channels (int): Number of input channels (3 for both xyz, direction)
        N_freqs (int): Number of frequency bands
        band_offset (int): Which frequency band to start at, where band 0 has period 2*pi
    ...
    def __init__(self, in_channels, N_freqs, band_offset=2, logscale=True, pre_rotate=False):
        super().__init__()
        ...
        if logscale:
            freq_bands = 2 ** (band_offset + torch.linspace(0, N_freqs - 1, N_freqs))
        else:
            freq_bands = 2 ** band_offset * torch.linspace(1, 2 ** (N_freqs - 1), N_freqs)
    ...
    def forward(self, x):
        ...
        if self.N_freqs > 0:
            ...
            for i, func in enumerate(self.funcs):
                ...
                if hasattr(self, "rot_mat"):
                    signal = self.freq_bands.to(x.device)[None, :, None, None] * x[:, None]
                    response = func(signal)
                    response = response.view(-1, self.N_freqs, input_dim, x.shape[-1])
                    response = response.mean(-1)
                else:
                    signal = self.freq_bands.to(x.device)[None, :, None] * x[:, None, :]
                    response = func(signal)
    ...
    def apply_annealing(self, out_bands):
        ...
        if self.alpha >= 0:
            ...
            window = alpha_freq - torch.arange(self.N_freqs, device=device)
...
class TimeEmbedding(nn.Module):
    ...
    def forward(self, frame_id=None):
        ...
        t_embed = torch.cat([coeff, inst_code], dim=-1)
