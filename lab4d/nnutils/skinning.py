...
class SkinningField(nn.Module):
    ...
    def forward(self, xyz, bone2obj, frame_id, inst_id):
        ...
        if hasattr(self, "delta_field"):
            ...
            # Clip logscale and shift to [-1.6, 1.6]. For reference, exp(1.6) ~= 4.953
            # Also multiply by 0.1, so that initialization is closer to identity
            logscale = torch.clip(0.1 * logscale, -1.6, 1.6)
            shift = torch.clip(0.1 * shift, -1.6, 1.6)

            dist2 = dist2 * logscale.exp() + shift
        else:
            delta = None
            
        # Larger dist2 (distance from xyz to bone) => Lower skinning weight
        skin = -dist2
        return skin, delta
    ...
    def draw_gaussian(self, articulation, edges, show_joints=False):
        ...
        articulation = dual_quaternion_to_se3(articulation).detach().cpu().numpy()
        ...
        for k, gauss in enumerate(gaussians):
            ...
            if show_joints:
                ...
                ellips.vertices *= gauss[None] * 0.5
        ...
        colormap_pad = np.ones((meshes.vertices.shape[0] - colormap.shape[0], 3), dtype=np.uint8)
