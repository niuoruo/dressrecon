...
def main():
    ...
    # io
    path_list = sorted(glob.glob(f"{args.testdir}/*-{args.data_class}-proxy.ply"))
    if len(path_list) == 0:
        print(f"no mesh found in {args.testdir} for {args.data_class}")
        return
    path_list = sorted(path_list, key=lambda x: int(x.split("/")[-1].split("-")[0]))
    outdir = f"{args.testdir}/renderings_proxy"
    ...
    # render
    frames = []
    for batch_idx, mesh_obj in tqdm.tqdm(mesh_dict.items(), desc=f"Rendering shape+cameras over optimization"):
        ...
        color = cv2.putText(
            color, f"batch: {batch_idx:02d}", (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (256, 0, 0), 2
        )
        frames.append(color)

    save_path = f"{outdir}/{args.data_class}"
