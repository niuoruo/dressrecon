...
class RenderFlags:
    ...
    flags.DEFINE_integer("render_res", 512, "rendering resolution")
    flags.DEFINE_string("viewpoint", "ref", "camera viewpoint, {ref,rot-elevation-degree,rot-60-0,...}")
    ...
    flags.DEFINE_integer("stride", 1, "render every `stride` frames")
    flags.DEFINE_float("render_scale", 1.0, "global scale factor to render the scene")
...
def construct_batch_from_opts(opts, model, data_info):
    ...
    if opts["freeze_id"] == -1:
        if opts["noskip"]:
            ...
            frameid = frameid[::opts["stride"]] + frameid[-1:]
        ...
        raise ValueError(f"frame id {opts['freeze_id']} out of range")

    if len(frameid_sub) == 0:
        raise ValueError("no frames specified to render")
    print(f"rendering frames: {frameid_sub} from video {video_id}")
    ...
    with torch.no_grad():
        ...
        intrinsics_fr = model.get_intrinsics()[frameid]
    ...
    if opts["viewpoint"] == "ref":
        ...
        raise ValueError(f"Unknown viewpoint type {opts.viewpoint}")
...
def render_batch(model, batch):
    ...
    print(f"rendering time: {time.time() - start_time:.3f}")
...
def render(opts, construct_batch_func, Trainer=Trainer):
    ...
    save_dir = make_save_dir(opts, sub_dir=f"renderings_{opts['inst_id']:04d}/{opts['viewpoint']}")
...
def main(_):
    opts = get_config()
    # os.system(f"python lab4d/render_intermediate.py --testdir logdir/{opts['seqname']}-{opts['logname']}")
