...
def construct_batch_from_opts_reanimate(opts, model, data_info):
    device = "cuda"
    # load motion data
    logroot, seqname, logname, motion_id = opts["logroot"], opts["seqname"], opts["logname"], opts["motion_id"]
    motion_path = f"{logroot}/{seqname}-{logname}/export_{motion_id:04d}/fg-motion.json"
    ...
    field2cam = torch.cat(se3_to_quaternion_translation(field2cam), dim=-1)
