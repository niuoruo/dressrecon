# Copyright (c) 2025 Jeff Tan, Gengshan Yang, Alex Lyons, Carnegie Mellon University.
import os
import sys
...
from lab4d.utils.flow_utils import flow_to_image


def to_cpu(img):
    if type(img) is tuple:
        img = tuple(to_cpu(x) for x in img)
...
def visualize_trajectory(all_verts, tag):
    ...
    trimesh.util.concatenate(meshes_concat).export(f"tmp/{tag}.ply")
