...
def fk_se3(local_rest_joints, so3, edges, to_dq=True, local_rest_coord=None):
    ...
    """
    assert local_rest_joints.shape == so3.shape, f"local_rest_joints {local_rest_joints.shape}, so3 {so3.shape}"
    shape = so3.shape
    ...
    local_to_parent = torch.cat([local_rmat, local_rest_joints[..., None]], dim=-1)
    local_to_parent = torch.cat([local_to_parent, identity_rt[..., -1:, :]], dim=-2)
...
def get_predefined_smpl_skeleton():
    ...
    SMPL_REST_JOINTS = adjust_human_rest_joints(
        SMPL_REST_JOINTS, SMPL_PARENT, leg_angle=0.0
    )
    return SMPL_PARENT, SMPL_SYMM_IDX, SMPL_REST_JOINTS


def get_predefined_human_skeleton():
    ...
    return HUMAN_PARENT, HUMAN_SYMM_IDX, HUMAN_REST_JOINTS


def get_predefined_quad_skeleton():
    QUAD_PARENT = {
        1: 0,  # spine 1
        13: 0,  # tail 1
        18: 0,  # left hip
    ...
    return QUAD_PARENT, QUAD_SYMM_IDX, QUAD_REST_JOINTS
...
def get_predefined_skeleton(skel_type):
    ...
    if skel_type == "smpl":
        edges, symm_idx, rest_joints = get_predefined_smpl_skeleton()
    elif skel_type == "human":
        edges, symm_idx, rest_joints = get_predefined_human_skeleton()
    elif skel_type == "quad":
        edges, symm_idx, rest_joints = get_predefined_quad_skeleton()
    else:
        raise ValueError(f"Unknown skeleton type {skel_type}")
