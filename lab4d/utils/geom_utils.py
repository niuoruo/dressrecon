...
def se3_vec2mat(vec):
    ...
    if torch.is_tensor(vec):
        ...
        if vec.shape[-1] == 6:
            rmat = dqtorch.axis_angle_to_matrix(vec[..., 3:6])
        else:
            rmat = dqtorch.quaternion_to_matrix(vec[..., [6, 3, 4, 5]])  # xyzw => wxyz
        tmat = vec[..., :3]
    else:
...
def se3_mat2vec(mat, outdim=7):
    ...
    if outdim == 7:
        ...
        raise ValueError(f"Invalid outdim {outdim}")
...
def eval_func_chunk(func, xyz, chunk_size, device=None):
...
def marching_cubes(
    ...
    # evaluate sdf
    sdf = eval_func_chunk(sdf_func, grid, chunk_size=chunk_size, device="cpu")
    ...
    if visibility_func is not None:
        vis = eval_func_chunk(visibility_func, grid, chunk_size=chunk_size, device="cpu")
