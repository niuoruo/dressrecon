...
def make_save_dir(opts, sub_dir="renderings"):
    ...
    save_dir = f"{opts['logroot']}/{logname}/{sub_dir}"
...
def save_vid(
    ...
    """
    print(f"Saving video to {outpath}{suffix}")
    ...
    if suffix == ".mp4":
        writer = imageio.get_writer(f"{outpath}{suffix}", fps=fps)
    elif suffix == ".gif":
        writer = imageio.get_writer(f"{outpath}{suffix}", duration=1000 // fps)
    else:
        raise NotImplementedError(suffix)

    for frame in frame_150:
        writer.append_data(resize_to_nearest_multiple(frame))
    writer.close()
...
def save_rendered(rendered, save_dir, raw_size, pca_fn):
    ...
    for k, v in rendered.items():
        ...
        # cv2.imwrite(f"{save_dir}/{k}.jpg", img_grid[:, :, ::-1])

        # save video
        frames = einops.rearrange(img_grid, "(m h) (n w) c -> (m n) h w c", h=h, w=w)
        frames = frames[:n]
        save_vid(f"{save_dir}/{k}", frames, fps=30, target_size=(raw_size[0], raw_size[1]))
